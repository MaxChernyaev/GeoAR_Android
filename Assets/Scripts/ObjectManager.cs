...
public class ObjectManager : MonoBehaviour
{
    ...
    {
        //GameObject.Find("SimpleFileBrowserCanvas").GetComponent<SimpleFileBrowser.FileBrowser>().OnPathChanged(Application.persistentDataPath);

        string PathToJson;
        SimpleFileBrowser.FileBrowser.ShowLoadDialog( ( paths ) => { PathToJson = ( paths[0] ); LoadObject(PathToJson); }, () => { PathToJson = ( "Canceled" ); }, SimpleFileBrowser.FileBrowser.PickMode.FilesAndFolders, false, Application.persistentDataPath, null, "Выбор JSON файла сохранения", "Выбрать");

        //LoadObject(PathToJson);
    }
    ...
    private void LoadObject(string PathToJson)
    {
        CommonSaveData commonSaveData = _saver.Load(PathToJson);
        FindObject = GameObject.Find("BasePlane(Clone)");

        if (commonSaveData == null)
        {
            Debug.Log("commonSaveData is null");
            TextLog.text = "Файл пустой";
            return;
        }
    ...
    private void SaveObjects()
    {
        ...
        string NameJson = "scene_" + System.DateTime.Now.ToString() + ".json";
        //SimpleFileBrowser.FileBrowser.ShowLoadDialog( ( paths ) => { PathToJson = ( paths[0] ); LoadObject(PathToJson); }, () => { PathToJson = ( "Canceled" ); }, SimpleFileBrowser.FileBrowser.PickMode.FilesAndFolders, false, Application.persistentDataPath, null, "Выбор JSON файла сохранения", "Выбрать");
        SimpleFileBrowser.FileBrowser.ShowSaveDialog( ( paths )  => { _saver.Save(commonSaveData, paths[0]); }, null, SimpleFileBrowser.FileBrowser.PickMode.Files, false, Application.persistentDataPath, NameJson, "Сохранение JSON файла сцены", "Сохранить" );
